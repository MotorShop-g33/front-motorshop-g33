...
import { Card, CardHeader, CardBody, CardFooter, Avatar, Image, Box, Wrap, WrapItem, ListItem } from "@chakra-ui/react"
...
export const ProfileAds = ({id, img, title, description, year, price, milage}: IProfileAds) => {
        ...
        <ListItem className="Ola">
            <Card w={312} h={450} boxShadow={"none"}>
                <CardHeader padding={"16px"} h={300} as={Link} to={`/product?ad=${id}`}>
                    <Box bg={"var(--fixed-white)"} h={150} display={"flex"} justifyContent={"center"}>
                ...
                <CardBody h={75} display={"flex"} justifyContent={"space-between"} padding={"16px"}>
...
export const ProfileCard = ({id, img, title, description, year, price, milage, user}: IProfileCard) => {
        ...
        <ListItem className="Ola" as={Link} to={`/product?ad=${id}`}>
            <Card w={312} h={400} boxShadow={"none"}>
                <CardBody padding={"16px"} h={300}>
                    <Box bg={"var(--fixed-white)"} h={152} display={"flex"} justifyContent={"center"}>
                ...
                <CardFooter h={65} display={"flex"} justifyContent={"space-between"} padding={"16px"}>
                    <Box display={"flex"} gap={"4"}>
